...
def pic_2_h5(filename, picfile_path, label=True):
    ...
    for dir in dirlist:  # 统计每一类文件夹中视频个数
        ...
        for frame in frames:
            ...
            image = image.reshape(1, 64, 64)  # 此处reshape，因为后续使用keras、cnn时，需要有图片的通道信息
            print(image.shape)
            sets_x.append(image)
        ...
        dset1 = f.create_dataset("test_set_x", data=data)       # 创建测试集数据标签
        dset2 = f.create_dataset("test_set_y", data=labels)
